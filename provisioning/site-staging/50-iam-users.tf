...
resource "aws_iam_user" "semmaphore-deploy-bas-style-kit-stage" {
    name = "semmaphore-deploy-bas-style-kit-stage"
}

# In-line policy for user
#
# This resource implicitly depends on the 'aws_iam_user.semmaphore-deploy-bas-style-kit-stage' resource
#
# AWS source: https://aws.amazon.com/iam/
# Terraform source: https://www.terraform.io/docs/providers/aws/r/iam_user_policy.html
resource "aws_iam_user_policy" "semmaphore-deploy-bas-style-kit-stage-semaphore-cd" {
    name = "semmaphore-deploy-bas-style-kit-stage-semaphore-cd"
    user = "${aws_iam_user.semmaphore-deploy-bas-style-kit-stage.name}"
...
#
# This resource implicitly depends on the 'aws_iam_user.semmaphore-deploy-bas-style-kit-stage' resource
# This resource explicitly depends on a manually defined 'automatons' group
#
# AWS source: https://aws.amazon.com/iam/
# Terraform source: https://www.terraform.io/docs/providers/aws/r/iam_group_membership.html
resource "aws_iam_group_membership" "automatons--semmaphore-deploy-bas-style-kit-stage" {
    name = "automatons--semmaphore-deploy-bas-style-kit-stage"
    group = "automatons"

    users = [
        "${aws_iam_user.semmaphore-deploy-bas-style-kit-stage.name}"
    ]
}
